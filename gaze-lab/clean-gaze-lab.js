...
(function(){
  ...
  const S={gaze:[],fix:[],sac:[],blink:[],events:[],selected:[],selecting:false,showInterval:false,videoUrl:null};
  ...
  function loadVideo(file){if(!file)return;if(S.videoUrl)URL.revokeObjectURL(S.videoUrl);S.videoUrl=URL.createObjectURL(file);$('sceneVideo').src=S.videoUrl}
  async function loadFolder(ev){
    try{
      ...
      S.selected=[];S.selecting=false;S.showInterval=false;
  ...
  function updateCalCounts(){text('intervalFixCount',candidates().length);text('selectedFixCount',S.selected.length+'/9')}
  ...
  function draw(){
    const c=$('overlayCanvas'),st=$('videoStage');if(!c||!st)return;const r=st.getBoundingClientRect(),dpr=devicePixelRatio||1;c.width=r.width*dpr;c.height=r.height*dpr;c.style.width=r.width+'px';c.style.height=r.height+'px';const ctx=c.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,r.width,r.height);
    const t=recTime();
    ...
    if(S.showInterval||S.selecting){const used=new Map(S.selected.map((f,i)=>[f.id,i+1]));candidates().forEach(f=>mark(ctx,f,used.has(f.id)?'#00c285':'#ff9f43',used.has(f.id)?String(used.get(f.id)):''));}
    text('videoTimeLabel',($('sceneVideo')?.currentTime||0).toFixed(3)+' s'); text('gazeTimeLabel',t.toFixed(3)+' s'); text('interactionModeLabel',S.selecting?'selezione 9 fissazioni':'navigazione');
  }
  function showInterval(){if(!validInterval())return status('Intervallo mancante','Inserisci secondi inizio e secondi fine calibrazione. Usa il punto decimale, es. 12.345.','warn');S.showInterval=true;S.selecting=false;S.selected=[];updateCalCounts();draw();status('Fissazioni intervallo','Mostro '+candidates().length+' fissazioni nel video principale.')}
  function startSelect(){if(!validInterval())return status('Intervallo mancante','Inserisci prima secondi inizio e secondi fine calibrazione.','warn');if(!candidates().length)return status('Intervallo vuoto','Nessuna fissazione trovata in questo intervallo.','warn');S.selected=[];S.showInterval=true;S.selecting=true;updateCalCounts();draw();status('Seleziona 9 fissazioni','Clicca: '+ORDER[0][1]+'.')}
  function nearestCandidate(p){let best=null,bd=Infinity;const used=new Set(S.selected.map(f=>f.id));for(const f of candidates()){if(used.has(f.id))continue;const d=Math.hypot(f.x-p.x,f.y-p.y);if(d<bd){bd=d;best=f}}return bd<=120?best:null}
  function clickOverlay(e){if(!S.selecting)return;const p=toVid(e);if(!p)return;const fx=nearestCandidate(p);if(!fx)return status('Nessuna fissazione vicina','Clicca più vicino a un punto arancione.','warn');S.selected.push(fx);updateCalCounts();draw();if(S.selected.length>=9){S.selecting=false;status('9 fissazioni selezionate','Premi Calcola correzione.');return}status('Fissazione salvata',S.selected.length+'/9. Ora clicca: '+ORDER[S.selected.length][1]+'.')}
  function clearCal(){S.selected=[];S.selecting=false;S.showInterval=false;updateCalCounts();draw();status('Calibrazione cancellata','Selezione delle fissazioni azzerata.')}
  function fitCal(){if(S.selected.length<4)return status('Calibrazione incompleta','Seleziona almeno i 4 estremi, meglio tutti e 9 i punti.','warn');status('Calibrazione pronta','Sono state memorizzate '+S.selected.length+' fissazioni. I 4 estremi derivano dai punti 1, 3, 7 e 9.');}
  function init(){
