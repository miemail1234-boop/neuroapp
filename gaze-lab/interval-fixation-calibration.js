...
(function(){
  ...
  function showAll(){
    if(!validInterval())return status('Intervallo mancante','Inserisci manualmente i secondi di inizio e fine calibrazione nei due campi. Usa il punto decimale, ad esempio 12.345.','warn');
    showInterval=true;selecting=false;selected=[];drawNow();status('Fissazioni calibrazione','Mostro nel video principale tutte le fissazioni comprese nell’intervallo: '+candidates().length+'.')
  }
  function startSelection(){const list=candidates();if(!validInterval())return status('Intervallo mancante','Inserisci manualmente i secondi di inizio e fine calibrazione.','warn');if(!list.length)return status('Intervallo vuoto','Imposta un intervallo che contenga le fissazioni dei 9 punti di calibrazione.','warn');selected=[];selecting=true;showInterval=true;drawNow();status('Seleziona 9 fissazioni','Clicca nel video principale la fissazione corrispondente a: '+labelForIndex(0)+'.')}
  ...
  function bindButtons(){
    const c=id('showIntervalFixBtn'),d=id('startNineFixSelectBtn'),e=id('clearCalibrationBtn'),f=id('fitCalibrationBtn');
    if(c)c.onclick=showAll;if(d)d.onclick=startSelection;if(e)e.onclick=clearSelection;if(f)f.onclick=()=>{applyPairs();if(typeof fitCal==='function')fitCal()};
    ['calStartSec','calEndSec'].forEach(k=>{const el=id(k);if(el&&!el.dataset.boundInterval){el.dataset.boundInterval='1';el.addEventListener('input',()=>{showInterval=validInterval();updateCount();drawNow()});el.addEventListener('change',()=>{showInterval=validInterval();updateCount();drawNow()})}})
  }
  function installUI(){
    const old=[...document.querySelectorAll('.panel-card')].find(sec=>sec.querySelector('.corner-grid'));
    if(old&&!old.dataset.intervalCalReady){old.dataset.intervalCalReady='1';old.innerHTML='<div class="panel-head"><h2>2. Intervallo di calibrazione</h2></div><p class="hint">Inserisci manualmente i secondi di inizio e di fine della fase in cui il soggetto guarda i 9 punti. Usa il punto decimale: esempio 12.345.</p><div class="form-grid"><label>Secondi inizio calibrazione<input id="calStartSec" type="number" step="0.001" placeholder="es. 12.345" /></label><label>Secondi fine calibrazione<input id="calEndSec" type="number" step="0.001" placeholder="es. 18.900" /></label></div><div class="button-row"><button id="showIntervalFixBtn" type="button" class="primary">Mostra fissazioni intervallo</button></div><div class="readout">Fissazioni nell’intervallo: <strong id="intervalFixCount">0</strong></div>'}
